...
/*void extract_a_file(char *archive_name, char *file_name) {
    FILE *read_ptr = fopen(archive_name, "rb");
    if (read_ptr != NULL) {
        unsigned int file_num = fgetc(read_ptr);
        for (int i = 0; i < file_num; i++) {
            unsigned long long file_size = get_file_size(read_ptr);
            unsigned char file_name_len = fgetc(read_ptr);
            int temp = 0;
            for (int = 0;
            i < min(strlen(file_name), file_name_len);
            i++)
            {
                temp += file_name[i] - fgetc(read_ptr);
            }
            if (temp == 0) {
