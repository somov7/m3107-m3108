...
int main(int argc, char *argv[]) {
    ...
    while(byte_sum < header_size){
        ...
        last_ptr = ftell(test_mp3);
        byte_sum += frame_size;
    }
    //printf("opa %x\n", last_ptr);
    fseek(test_mp3, last_ptr, SEEK_SET);
    if(do_set){
        //printf("file ptr: %x\n", ftell(test_mp3));
...
//        for(int i = 0; i < 4; i++){
//            printf("%x %x\n", ghost_frame.id[i], ghost_frame.size[i]);
//        }
        fwrite(&ghost_frame, sizeof(char), 10, test_mp3);
        //printf("file ptr: %x\n", ftell(test_mp3));
        fflush(test_mp3);
        char x[1];
        x[0] = '\0';
