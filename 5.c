...
void print_file()
{
    static int it_output=0;
    it_output++;
    FILE *fout;
    fout=fopen(merge(path, merge("//", merge(get_str(it_output), ".bmp"))), "wb");
...
int main(int argc, char** argv)
{
    ...
    // 5.exe --input 12.bmp --output D:\\testing --max_inter 50 --dumb_freg 10
    if (error)
    {
        printf("--ERROR--\n____________________\n");
        return 0;
