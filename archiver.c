...
void special_extract(char *file_name, FILE *file_arc){
    char buffer[BufferSize] = {'\0'};
    uint64_t number_of_bytes;
    uint64_t num_of_bytes_to_read;
    uint64_t need_to_read;
    ...
    while(1) {
        ...
        number_of_bytes = fread(file_header.f_name, sizeof (char), file_header.name_len, file_arc);
        if (number_of_bytes == 0) break;
        if (strcmp(file_header.f_name, file_name) == 0){
            check = 1;
            printf("here it is! %s\n", file_header.f_name);
        }
        else{
...
int main(int argc, char *argv[]) {
    ...
    && (strcmp(argv[3], "--specExtract") != 0) && argc > 4)){
    ...
    uint16_t spec_extraction_flag = 0;
...
//        printf("%s\n", argv[i]);
        ...
        else if (strcmp(argv[i], "--specExtract") == 0){
            target_file = argv[++i];
            spec_extraction_flag = 1;
        }
...
//    }
    ...
    else if (spec_extraction_flag){
        arc_file = fopen(arch_name, "rb");
        if (arc_file == NULL){
            printf("can't open file %s\n", arch_name);
            return 1;
        }
        special_extract(target_file, arc_file);
        fclose(arc_file);
    }
