...
const int cc = 1e9; //основание системы счисления
...
uint1024_t from_uint(unsigned int x) 
{
    ...
    uint1024_t t;
    t.int32 = malloc(size * sizeof(int32_t));//выделяем память размером size * sizeof(int32_t)
    t.size = size;
    t.int32[0] = x % cc;
...
{
    int size = x.size;
    printf("%d", !size ? 0 : x.int32[size - 1]); //выводим значимые цифры без 9 нулей
    for (int index = size - 2; index >= 0; --index) //выводим результат:значимые цифры печатаются в конце массива
...
void scanf_value(uint1024_t* x)//заполнение полного массива
{
    ...
    for (int index = len, j = 0; index > 0; index -= 9, j++)
    {
        ...
        result.int32[j] = (atoi(index >= 9 ? str + index - 9 : str)); //беру 9 цифр с конца, если индекс меньше 9, перехожу на оставшуюся строку
    }
    *x = result;
}
