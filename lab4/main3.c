...
int main (int argc, char *argv[]) {
    ...
    FILE *fin, *fout;
    int flag_set = 0, flag_show = 0, flag_get = 0, size = 0, cnt = 0;
    ...
    while ((fread(&frame, 1, 11, fin))) {
        cnt++;
        if (bytes_to_int(frame.size, 8) == 0)
            break;
    }
    ...
    while ((fread(&frame, 1, 11, fin)) && ftell(fin) <= cur_size) {
        ...
        if (flag_set && !strcmp(frame.id, prop_name) && size < cnt) {
            ...
            temp2 = (char *) malloc(end - position + 1);
            fread(temp2, 1, end - position + 1, fin);

            fseek(fin, 0, SEEK_SET);
            fwrite(&header, 1, 10, fin);
            fwrite(temp, 1, position - characters_in_frame - (cur_frame_size - 1), fin);
            fwrite(&frame, 1, 11, fin);
            fwrite(prop_value, 1, strlen(prop_value), fin);
            fwrite(temp2, 1, end - position + 1, fin);

            free(temp);
            free(temp2);
        }
        if (flag_set && strcmp(frame.id, prop_name) && size > cnt) {
            int i, position, end, new_frame_size, new_header_size;
            char *temp, *temp2;
            id3_frame new_frame;
            ...
            position = ftell(fin);
            fseek(fin, 10, SEEK_SET);
            temp = (char *) malloc(position - 10 + 1);
            fread(temp, 1, position - 10 + 1, fin);
            ...
            fseek(fin, 0, SEEK_SET);
            fwrite(&header, 1, 10, fin);
            fwrite(&new_frame, 1, 11, fin);
            fwrite(prop_value, 1, strlen(prop_value), fin);
            fwrite(temp, 1, position - 10 + 1, fin);
            fwrite(temp2, 1, end - position + 1, fin);
            free(temp);
            free(temp2);
        }
