...
int main(int argc, char *argv[]) {
    ...
    while(byte_sum < header_size){
        ...
        char frame_encoding;
        frame_header frame1;
        fread(&frame1, sizeof(char), sizeof(frame1), mp3);
        for (int i = 0; i < 4; i++){
        ...
        frame_size = frame1.size[0] * 0x200000 +   //2^21
                     ...
                     frame1.size[3];
        byte_sum += 10;
        fread(&frame_encoding, sizeof(char), sizeof(frame_encoding), mp3);
        if (do_set && (strcmp(field_ptr, frame_id) == 0)){
            into_frame_size(value_size + 1, frame1.size);
            
            uint32_t file_ptr = ftell(mp3);
            
            fseek(mp3, frame_size - 1, SEEK_CUR);
            
            char *buf = (char*) calloc(header_size - file_ptr - value_size, sizeof (char));
            fread(buf, sizeof (char), header_size - file_ptr - value_size, mp3);
            fflush(mp3);
            char x[1];
            x[0] = '\0';
            fseek(mp3, file_ptr - 1, SEEK_SET);
            fwrite(x, sizeof(char), 1, mp3);
            fflush(mp3);
            fseek(mp3, file_ptr, SEEK_SET);
            fwrite(value_ptr, sizeof(char), value_size, mp3);
            fwrite(buf, sizeof (char), header_size - file_ptr - value_size, mp3);
            fseek(mp3, file_ptr - 7, SEEK_SET);
            fwrite(frame1.size, sizeof (char), sizeof (frame1.size), mp3);
            fflush(mp3);
            fclose(mp3);
            free(buf);
            return 0;
        ...
        if (frame_encoding == 0 || frame_encoding == 3){
            ...
            unsigned char bom[3];
            fread(&bom, sizeof(char), 2, mp3);
            bom[2] = '\0';
            if (do_show){
                wchar_t temp[frame_size];
                fgetws(temp, (int)frame_size / 2, mp3);
                printf("%s ", frame_id);
                wprintf(L"%ls\n", temp);
            }
            else if (do_get && (strcmp(field_ptr, frame_id) == 0)){
    ...
    fseek(mp3, last_ptr, SEEK_SET);
    if(do_set){
        frame_header ghost_frame;
        ghost_frame.flags[0] = '\0';
        ghost_frame.flags[1] = '\0';
        memcpy(ghost_frame.id, field_ptr, 4);
        into_frame_size(value_size + 1, ghost_frame.size);
        fwrite(&ghost_frame, sizeof(char), 10, mp3);
        fflush(mp3);
        char x[1];
        x[0] = '\0';
        fwrite(x, sizeof(char), 1, mp3);
