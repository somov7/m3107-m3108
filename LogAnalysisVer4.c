...
#include <time.h>
#include <inttypes.h>
#define MaxStrLen 2049 // в принципе 1000 хватает
#define MaxNumOfWords 2048 // а здесь 100
...
int detect_month(char *mon){
    ...
    for (int i = 0; i < 12; i++){
        if (strcmp(mon, months[i]) == 0){
            return i; // в структуре нумерация месяцев с нуля?
        }
    }
}
...
//    int num = 0;
//    char *res;
//    char **temp = malloc(MaxNumOfWords);
//    res = strtok(str, sep);
//    while(res != NULL){
//        temp[num] = malloc(strlen(res) + 1);
//        strcpy(temp[num], res);
//        num++;
//        res = strtok(NULL,sep);
//    }
